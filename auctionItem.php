...
class AuctionItem
{
    ...
    // Constructor
    public function __construct($itemID, $title, $description, $startTime, $endTime, $startingPrice, $highestBid, $sellerProfile, $buyerProfile, $categoryID, $conn)
    {
        $this->itemID = $itemID;
        $this->title = $title;
    ...
    // Method to add new auction item
    public function createItem($title, $format, $releaseYear, $author_Artist, $isbn_Upc, $runtime_Duration, $genre, $language, $publisher_Label, $conn)
    {
        // Query to insert media into database
        $sql = "INSERT INTO auction_item (title, description, startTime, endTime, startingPrice, sellerProfile, categoryID) VALUES (?, ?, ?, ?, ?, ?, ?)";
